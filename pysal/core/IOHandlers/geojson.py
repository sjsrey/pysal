...
class GeoJson(Tables.DataTable):
...
def _parse(source):
    ...
    """
    source can be a uri, local file or stream
...
if __name__ == '__main__':
    import pysal
    fpath = pysal.examples.get_path("columbus.json")


    fpath = pysal.examples.get_path("columbus.json")
    gj = GeoJsonHandler(fpath)
    s = json.dumps(gj.content)
    gjs = GeoJsonHandler(s)

    # virginia
    fpath = pysal.examples.get_path("virginia.json")

    jg = GeoJsonHandler(fpath)
