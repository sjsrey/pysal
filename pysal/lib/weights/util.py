...
def hexLat2W(nrows=5, ncols=5, **kwargs):
    ...
    **kwargs   : keyword arguments
    ...
    Examples
    --------
    >>> from pysal.lib.weights import lat2W, hexLat2W
    >>> w = lat2W()
    >>> w.neighbors[1]
    [0, 6, 2]
    >>> w.neighbors[21]
...
def order(w, kmax=3):
    ...
    Examples
    --------
    >>> from pysal.lib.weights import order, Rook
    >>> import pysal.lib
...
def higher_order_sp(w, k=2, shortest_path=True, diagonal=False, **kwargs):
    ...
    **kwargs      : keyword arguments
    ...
    >>> from pysal.lib.weights import lat2W, higher_order_sp
    >>> w25 = lat2W(5,5)
    >>> w25.n
    25
    >>> w25[0] == {1: 1.0, 5: 1.0}
...
def w_local_cluster(w):
    ...
    Examples
    --------
    >>> from pysal.lib.weights import lat2W, w_local_cluster
...
def shimbel(w):
    ...
    Examples
    --------
    >>> from pysal.lib.weights import lat2W, shimbel
    >>> w5 = lat2W()
...
def full2W(m, ids=None, **kwargs):
    ...
    **kwargs   : keyword arguments
    ...
    Examples
    --------
    >>> from pysal.lib.weights import full2W
    >>> import numpy as np
...
def WSP2W(wsp, **kwargs):
    ...
    **kwargs                : keyword arguments
    ...
    Examples
    --------
    >>> from pysal.lib.weights import lat2W, WSP, WSP2W
...
def lat2SW(nrows=3, ncols=5, criterion="rook", row_st=False):
    ...
    >>> from pysal.lib.weights import lat2SW
    >>> w9 = lat2SW(3,3)
    >>> w9[0,1] == 1
    True
    >>> w9[3,6] == 1
...
def attach_islands(w, w_knn1, **kwargs):
    ...
    **kwargs     : keyword arguments
    ...
    Examples
    --------
    >>> from pysal.lib.weights import lat2W, Rook, KNN, attach_islands
    >>> import pysal.lib
    >>> w = Rook.from_shapefile(pysal.lib.examples.get_path('10740.shp'))
    >>> w.islands
    [163]
    >>> w_knn1 = KNN.from_shapefile(pysal.lib.examples.get_path('10740.shp'),k=1)
...
def nonplanar_neighbors(w, geodataframe, tolerance=0.001, **kwargs):
    ...
    **kwargs:  keyword arguments
    ...
    >>> wnp = pysal.lib.weights.nonplanar_neighbors(w, df)
    >>> wnp.islands
    []
    >>> w.neighbors[0]
    []
...
def fuzzy_contiguity(gdf, tolerance=0.005, buffering=False, drop=True, **kwargs):
    ...
    **kwargs: keyword arguments
    ...
    >>> import geopandas as gpd
    >>> rs = pysal.lib.examples.get_path('map_RS_BR.shp')
    >>> rs_df = gpd.read_file(rs)
    >>> wq = pysal.lib.weights.Queen.from_dataframe(rs_df)
    >>> len(wq.islands)
    29
    >>> wq[0]
    {}
